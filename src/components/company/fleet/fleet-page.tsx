...
import { TYPE_LABELS, STATUS_VARIANTS, BUS_TYPES } from './data'
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold">Quan Ly Xe</h1>
                    <p className="text-sm text-muted-foreground">Danh sach phuong tien cua cong ty</p>
                </div>
                <Button onClick={openAdd}>
                    <Plus className="h-4 w-4" /> Them xe
                </Button>
            </div>

            <div className="grid gap-3 sm:grid-cols-4">
                {[
                    { label: 'Tong xe', value: stats.total, color: 'text-foreground' },
                    { label: 'Hoat dong', value: stats.active, color: 'text-green-500' },
                    { label: 'Bao duong', value: stats.maintenance, color: 'text-orange-500' },
                    { label: 'Tam dung', value: stats.inactive, color: 'text-muted-foreground' },
                ].map(s => (
                    <Card key={s.label}>
            ...
            <div className="flex flex-wrap gap-3">
                <div className="relative flex-1 min-w-48 max-w-sm">
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Tim bien so, ten xe..."
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            <CardContent className="p-4 space-y-3">
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">{TYPE_LABELS[bus.type]}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="flex justify-end pt-1">
                                    <button onClick={() => openEdit(bus)}
                                        ...
                                        <Pencil className="h-3 w-3" /> Chinh sua
                                    </button>
                                </div>
                            </CardContent>
                        </Card>
                ...
                {filtered.length === 0 && (
                    <div className="col-span-full py-16 text-center text-muted-foreground">
                        ...
                        <p>Khong tim thay xe nao</p>
                    </div>
                )}
            </div>

            <Dialog open={dialogOpen} onClose={closeDialog} title={selected ? 'Chỉnh sửa xe' : 'Thêm xe mới'}>
                <VehicleForm
